...
model = AutoModelForCausalLM.from_pretrained(model_dir, device_map="cpu", trust_remote_code=True, bf16=True).eval()
...
def process_image(image_path):
    ...
    query = tokenizer.from_list_format([
        {'image': image_path},
        {'text': 'Draw a bounding box around each visible damage in the image and label it with the name of the damage only.'},
    ])
...
def process_image_text(image_path):
    query = tokenizer.from_list_format([
        {'image': image_path},
        {'text': 'Identify and name the type of damage visible in the image. Respond only with the damage type.'},
    ])
    response, history = model.chat(tokenizer, query=query, history=None)
...
def get_recommendation_for_damage(damage_type):
    recommendations = {
        "hole": "Colmater le trou avec un matériau adapté (mastic, résine ou ciment) selon le support, puis poncer et repeindre.",
        "mold": "Nettoyer avec un produit antifongique, traiter l'humidité à l'origine et ventiler la zone affectée.",
        "crack": "Analyser la profondeur de la fissure ; si structurelle, faire appel à un professionnel. Sinon, combler avec un enduit adapté.",
        "scratche": "Poncer légèrement la surface, appliquer une couche d’apprêt si nécessaire, puis repeindre pour protéger le matériau.",
        "rust": "Enlever la rouille par ponçage ou traitement chimique, appliquer un antirouille, puis repeindre la zone traitée.",
        "stain": "Nettoyer avec un détergent adapté au type de surface. Si la tache est incrustée, envisager une remise en état ou un revêtement.",
    }

    return recommendations.get(damage_type.lower(), "Consulter un professionnel pour évaluer ce type de dommage.")
...
def analyze_zip(zip_path):
    ...
    response, output_image_path = process_image(largest_image)
    ...
    response = process_image_text(largest_image)
    
    damage_type_match = re.search(r'\b(hole|mold|crack|scratche|rust|stain)\b', response.lower())
    damage_type_cleaned = damage_type_match.group(1) if damage_type_match else response.lower().strip()

    recommendation = get_recommendation_for_damage(damage_type_cleaned)
    
    return response, output_image_path, Width, Height, Position #, recommendation
